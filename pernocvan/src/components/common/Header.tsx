...
export const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-[9999] bg-background border-b border-border relative">
      ...
      {isMenuOpen && (
        <div className="md:hidden flex flex-col px-4 pt-4 pb-6 border-t border-border bg-background space-y-4 shadow-lg absolute w-full left-0 top-[100%] z-50">
        ...
        {session ? (
            ...
            {isAdmin && !ocultarPorRuta && (
              <div className="flex flex-col gap-3 pb-2 border-b border-border/50">
                <Button 
                  variant="outline" 
                  asChild 
                  className="w-full h-12 p-4 justify-start text-[#e03b4b] hover:text-[#e03b4b] hover:bg-red-500/10 text-base"
                >
                  <Link to="/admin/dashboard"><LayoutDashboard className="h-5 w-5 mr-3" />Panel Admin</Link>
                </Button>
                <Button 
                  variant="outline" 
                  asChild 
                  className="w-full h-12 p-4 justify-start text-[#e03b4b] hover:text-[#e03b4b] hover:bg-red-500/10 text-base"
                >
                  <Link to="/admin"><Users className="h-5 w-5 mr-3" />Gestión de Usuarios</Link>
                </Button>
              </div>
            )}
            <Button 
              variant="outline" 
              asChild 
              className="w-full h-12 p-4 justify-start text-foreground text-base"
            >
              <Link to="/profile"><User className="h-5 w-5 mr-3" />Mi Perfil</Link>
            </Button>
            <Button 
              variant="destructive" 
              onClick={handleLogout} 
              className="w-full h-12 p-4 justify-start cursor-pointer text-base"
            >
          ...
          <div className="flex flex-col gap-3">
            <Button 
              ...
              className="w-full h-12 p-4 justify-start text-foreground text-base"
            >
              <Link to="/login"><LogIn className="h-5 w-5 mr-3" />Iniciar Sesión</Link>
            </Button>
            <Button 
              asChild 
              className="w-full h-12 p-4 justify-start text-primary-foreground text-base"
            >
              <Link to="/signup"><UserPlus className="h-5 w-5 mr-3" />Registrarse</Link>
            </Button>
          </div>
        )}
      </div>
